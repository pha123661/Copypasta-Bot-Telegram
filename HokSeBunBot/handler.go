...
func handleCallbackQuery(bot *tgbotapi.BotAPI, CallbackQuery *tgbotapi.CallbackQuery) {
	...
	if CallbackQuery.Data == "NIL" {
		...
		Entity := Queued_Overwrites[CallbackQuery.Data]
		if Entity.Done {
			return
		}
		Entity.Done = true
		...
		// send response to user
		replyMsg = tgbotapi.NewMessage(CallbackQuery.Message.Chat.ID, fmt.Sprintf("更新複製文「%s」成功，自動生成的摘要如下：「%s」", Entity.Keyword, Sum))
		replyMsg.ReplyToMessageID = CallbackQuery.Message.MessageID
		if _, err := bot.Send(replyMsg); err != nil {
			log.Println("[CallBQ]", err)
		}
